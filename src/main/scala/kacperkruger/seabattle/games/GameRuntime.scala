...
import kacperkruger.seabattle.consoles.Console
import kacperkruger.seabattle.games.phases.{PlacingPhase, PlayingPhase}
import kacperkruger.seabattle.games.status.{OnGoing, WaitingForPlayers, Won}
import kacperkruger.seabattle.players.Player
import kacperkruger.seabattle.ships.*
...
final class GameRuntime(console: Console):
  ...
  def printOpponentBoard(game: Game, player: Player): IO[Unit] = {
    import kacperkruger.seabattle.boards.ShowBoardWithoutShips.given
    for {
  ...
  private def loop(game: Game): IO[Unit] = for {
    _ <- game.status match {
      case WaitingForPlayers =>
        console.printLine("Waiting for more kacperkruger.seabattle.players...")
